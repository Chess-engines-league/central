        ...
        try {
            ...
            for (int i = 1; i <= 2000; i++) {
                if (game.getMoves().size() % 2 == 0) {
                    ...
                    Optional<GameResult> result = halfMove(game, game.getWhite(), moveLimit, Side.WHITE);
                    if (result.isPresent()) {
                        return result.get();
                    }
                } else {
                    Optional<GameResult> result = halfMove(game, game.getBlack(), moveLimit, Side.BLACK);
                    if (result.isPresent()) {
                        return result.get();
                    }
                }
